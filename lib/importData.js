...
async function syncAll() {
  ...
  const slots = bracket.importBracket(fixtures);
  const topscorers = await api.getTopScorers();
  const topassists = await api.getTopAssists();
  const players = scorers.importPlayers(topscorers, topassists);
  return {
